...
df_grouped = df_filtrado.groupby('PERIODO').agg({
    'CONSUMO': 'sum',
    'IMPORTE': 'sum'
}).rename(columns={'CONSUMO': 'CONSUMO TOTAL', 'IMPORTE': 'IMPORTE TOTAL'})

# Escalar y redondear a miles con 1 decimal
df_grouped['CONSUMO TOTAL'] = (df_grouped['CONSUMO TOTAL'] / 1000).round(1)
df_grouped['IMPORTE TOTAL'] = (df_grouped['IMPORTE TOTAL'] / 1000).round(1)
...
st.caption("Nota: Los valores de consumo e importe están expresados en miles. Por ejemplo, 12.5 equivale a 12,500 kWh.")
...
# Modelo Prophet
    ...
    st.info("""
        ...
        Como eleva los errores al cuadrado, **castiga mucho más los errores grandes**.  
        🔍 *Ejemplo:* Si un mes el modelo se equivoca por 50 unidades y otro mes por 5, el MSE dará mucho peso al error de 50.

        - **📏 MAE (Error Absoluto Medio)**  
        Mide en promedio cuántas unidades se equivoca el modelo, sin importar si fue más o menos.  
        🧮 *Ejemplo:* Si estás prediciendo consumo de electricidad en kWh y el MAE es 8.4, eso significa que el modelo se equivoca, en promedio, **8.4 kWh por mes**.

        - **📊 RMSE (Raíz del Error Cuadrático Medio)**  
        Similar al MSE, pero en las mismas unidades que los datos.  
        Sensible a errores grandes como el MSE, pero más fácil de interpretar.  
        🔢 *Ejemplo:* Si el RMSE es 12.5, es como decir que, en promedio, **la predicción se desvía unos 12.5 kWh del valor real**.

        - **📌 MAPE (Error Porcentual Absoluto Medio)**  
        Mide en porcentaje cuánto se equivoca el modelo en promedio.  
        Ideal para explicar los errores a personas no técnicas.  
        🧠 *Ejemplo:* Si el MAPE es 6.3%, significa que **el modelo se equivoca en promedio un 6.3% con respecto al valor real**.
        """)